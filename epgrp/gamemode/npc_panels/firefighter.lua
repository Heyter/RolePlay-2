...
DIALOG["NPC_FireHello"] = function( frame ) 
	local tbl = {
		{
			npcsay = "Guten Tag.\nWas kann ich für sie tun?",
			options = {
                ["Ich möchte der Feuerwehr angehören."] = function( frame ) 
					CanDoJob( TEAM_FIRE, function( canDo )
						if canDo then
							frame:GoToStage( 2 )
						else
							frame:Close()
...
DIALOG["NPC_FireInTeam"] = function( frame ) 
	local tbl = {
		{
