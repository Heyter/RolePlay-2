...
function ENT:Initialize() self:SetModel("models/vcmod/vcmod_wrenchset.mdl") self:PhysicsInit(SOLID_VPHYSICS) self:SetMoveType(MOVETYPE_VPHYSICS) self:SetSolid(SOLID_VPHYSICS) self:SetUseType(SIMPLE_USE) local phys = self:GetPhysicsObject() if phys:IsValid() then phys:Wake() end end
...
function ENT:Touch(ent)
	...
	if VC.RepairHealth then VC.RepairHealth(ent, ent.VC_MaxHealth/4) end
