...
function PANEL:Init()	
	...
	self.JailSlider:SetMax(RHC_MaxJailYears)
	...
	self.JailButton.DoClick = function() net.Start("RHC_jail_player") net.WriteEntity(self.APlayer) net.WriteFloat(self.JailSlider:GetValue()) net.SendToServer() self:Remove() end		
...
function PANEL:Paint(W,H)
	...
	draw.SimpleText(self.JailNick, "Trebuchet18", W/2, H*0.35, Color(255,255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )   
end

local Width, Height = 200, 125
function PANEL:PerformLayout()
	...
	self.JailSlider:SetPos(5,Height/2)
	self.JailSlider:SetSize(Width-10,20)
...
function PANEL:SetBailPlayer(Player)
	...
	if Player.ArrestTime and Player.HCArrestedBy then
		...
		self.BailPrice = Time*RHC_BailPricePerYear
	end
