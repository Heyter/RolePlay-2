...
local title = "GExtension :: Warnings"
			...
			function Admin_Set(warnings, players){
				...
				if(players && warnings){
					...
					for (var key in players){
						...
						var html_tab = '<li class="' + tclass + '"><a href="#tab_' + key + '" data-toggle="tab"><i class="fa fa-' + icon + '"></i> ' + nick + '<span class="pull-right">' + html_warncount + ' <span onclick="OpenWarnModal(\'' + key + '\', \'' + nick + '\');" style="cursor: pointer;" class="badge">+</span></span></a></li>';
...
function GExtension:CreateWarningsPanel()
	...
	GExtension_Warnings_Panel:SetTitle("GExtension :: Warnings")
...
net.Receive("GExtension_Net_WarningsPanelShow", function()
	if IsValid(GExtension_Warnings_Panel) then
		GExtension_Warnings_Panel:Hide()
	else
