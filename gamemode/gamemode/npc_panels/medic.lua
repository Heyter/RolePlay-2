...
DIALOG["NPC_MedicHello"] = function( frame ) 
	local tbl = {
		{
			npcsay = "Guten Tag.\nWas kann ich für sie tun?",
			options = {
                ["Ich möchte dem Notdienst angehören."] = function( frame ) 
					CanDoJob( TEAM_MEDIC, function( canDo )
						if canDo then
							frame:GoToStage( 2 )
						else
							frame:Close()
...
DIALOG["NPC_MedicInTeam"] = function( frame ) 
	local tbl = {
		{
