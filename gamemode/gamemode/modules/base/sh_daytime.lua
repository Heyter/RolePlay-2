...
if SERVER then
    ...
    // convars.
    daylight.dayspeed = CreateConVar( 'daytime_speed', '1000', { FCVAR_REPLICATED , FCVAR_ARCHIVE , FCVAR_NOTIFY } );
    ...
    local morning = true
    function daylight:CalcDayTimeClock( minute )
        local m = 0
        local h = 0
        ...
        if h > 11 then morning = false else morning = true end

        return h, math.Round( m / 2 ), morning
    end
    
    util.AddNetworkString( "DayNight_SendClock" )
    function daylight:SendCurClock()
        local hour, minute, m = self:CalcDayTimeClock( self.minute )
        net.Start( "DayNight_SendClock" )
            net.WriteString( hour )
            net.WriteString( minute )
            net.WriteString( tostring(m) )
        net.Send( player.GetAll() )
    end
    
    local lastm = 0
    function daylight:RefreshTime()
        local hour, minute, morning = self:CalcDayTimeClock( self.minute )
        if minute == lastm then return false end
        
        daylight:SendCurClock()
    end
    ...
    hook.Add( 'PlayerAuthed', 'DayLight_Send_Clock', function( ply ) timer.Simple( 1, function() daylight:SendCurClock() end ) end )
end

if CLIENT then
    net.Receive( "DayNight_SendClock", function( data )
        local hour, minute, morning = tonumber(net.ReadString()), tonumber(net.ReadString()), tobool(net.ReadString())
        daylight.hour = hour
        daylight.minute = minute
        daylight.morning = morning
    end)
    
    hook.Add( "HUDPaint", "DayNight_HUDPaint", function()
        if LocalPlayer():GetRPVar( "playtime" ) == nil then return end
        
        local h_string = daylight.hour
        local m_string = daylight.minute
        if daylight.hour < 10 then
            h_string = "0" .. daylight.hour
        end
        if daylight.minute < 10 then
            m_string = "0" .. daylight.minute
        end
        draw.SimpleText( h_string .. ":" .. m_string, "RPNormal_45", 15, 15, Color( 255, 255, 255, 255 ) )
        
        if daylight.morning then
            draw.SimpleText( "Morgens", "RPNormal_23", 15, 1, Color( 0, 204, 255, 255 ) )
        end
        
        if daylight.hour < 18 and !(daylight.morning) then
            draw.SimpleText( "Mittags", "RPNormal_23", 15, 1, Color( 255, 153, 0, 255 ) )
        elseif daylight.hour > 17 and !(daylight.morning) then
            draw.SimpleText( "Abends", "RPNormal_23", 15, 1, Color( 180, 0, 0, 255 ) )
        end
        
        // Let's draw the playtime :)
        draw.SimpleText( ConvPlaytime( LocalPlayer():GetRPVar( "playtime" ) ), "RPNormal_24", 15, 55, Color( 255, 255, 255, 255 ) )
    end)
end
