...
GLOBAL_EMITTER = ParticleEmitter( Vector( 0, 0, 0 ), false )
SMOKE_EMITTER = ParticleEmitter( Vector( 0, 0, 0 ), false )


function EFFECT:Init( data )

	local disablesmoke = 1 --GetConVar( "gmnrp_options_dsmoke" ):GetInt()
	...
	if !(tonumber(disablesmoke) == 1) then
		// Fire Parameters
		p = GLOBAL_EMITTER:Add( "effects/perp2_flame",Pos);
		// Smoke Parameters
		s = SMOKE_EMITTER:Add("effects/tornado_smoke", Pos + Vector(0, 0, 70));
		// Fiery Smoke Parameters
		e = SMOKE_EMITTER:Add("effects/tornado_smoke", Pos + Vector(0, 0, 40));
	else
		// Fire Parameters
		p = GLOBAL_EMITTER:Add( "effects/perp2_flame",Pos);
	end;
	...
	if tonumber(disablesmoke) == 0 then
