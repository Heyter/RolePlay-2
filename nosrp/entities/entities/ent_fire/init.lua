...
function ENT:HitByExtinguisher ( ByWho, IsHose )
	if IsHose then
		...
		if ByWho and ByWho:IsValid() and ByWho:IsPlayer() and ByWho:Team() == TEAM_FIRE then
			self.ExtinguisherLeft = self.ExtinguisherLeft - 2;
		elseif ByWho and ByWho:IsValid() and ByWho:IsPlayer() then
			self.ExtinguisherLeft = self.ExtinguisherLeft - 1;
		end
	end
	
	if self.ExtinguisherLeft <= 0 then
		if ByWho and ByWho:IsValid() and ByWho:IsPlayer() && ByWho:Team() == TEAM_FIRE then
			if IsHose then
				ByWho:AddCash(5);
        ...
        if ByWho:Team() == TEAM_FIRE then
            ByWho:SetSkill( "Erste Hilfe", ByWho:GetSkill( "Erste Hilfe" ) + 0.01 )
            ByWho:SetSkill( "Ruf", ByWho:GetSkill( "Ruf" ) + 0.06 )
        end
		self:KillFire();
	end
end
...
function ENT:Think ( )
	...
	if self.LastDamage + .1 < CurTime() then
		...
		for k , v in pairs(CloseEnts) do
			if (v:GetClass() == 'prop_physics' or v:GetClass() == 'ent_item' or v:GetClass() == 'nosrp_prop' or v:GetClass() == "prop_clock" or v:GetClass() == "ent_pot" or v:GetClass() == "ent_shroom" or v:GetClass() == "item_doorbuster") and !v:GetTable().UnBurnable then
			...
			elseif v:IsPlayer() and v:Alive() and v:GetPos():Distance(self:GetPos()) < 70 then
				if v:Team() != TEAM_FIRE then
					v:TakeDamage(2, v, self);
					
					if v:GetPos():Distance(self:GetPos()) < 50 and v:Team() != TEAM_FIRE then
